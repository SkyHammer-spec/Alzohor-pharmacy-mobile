...
async function findFileIdByName(filename) {
  ...
  // orderBy=modifiedTime desc: if the desktop sync ever leaves more than one
  // file with this name in the folder (e.g. a delete+recreate cycle caught
  // mid-flight), always pick the most recently modified one rather than
  // whatever Drive happens to list first — otherwise the phone can silently
  // fetch a stale duplicate and show an old generatedAt as if it were fresh.
  const url = `${DRIVE_API_BASE}/files?q=${q}&key=${API_KEY}&orderBy=modifiedTime desc&fields=files(id,modifiedTime)`;
...
// Minimal shape check for admin-snapshot.json — catches a partial/failed
// write on the desktop side (or an old snapshot format) before we trust it
// or let it overwrite a perfectly good cached copy. Deliberately loose: we
// only check that the top-level sections exist, not their full internal
// shape, so we don't need updating every time a field is added.
function isValidSnapshot(data) {
  return !!data && typeof data === 'object' && data.alerts && typeof data.alerts === 'object';
}
...
async function fetchWithFallback(filename) {
  try {
    ...
    if (!isValidSnapshot(data)) {
      // Don't cache it, and don't hand it to the UI — fall back to whatever
      // we had before rather than let a bad write clobber "last known good".
      throw new Error('Fetched snapshot is missing expected data (partial write?)');
    }
    saveToCache(filename, data);
    // Use the desktop's own sync timestamp (data.generatedAt) rather than
    // "now" — "now" is just when the phone happened to fetch, not when the
    // desktop last actually pushed fresh data.
    return { data, fromCache: false, timestamp: data.generatedAt || new Date().toISOString() };
  } catch (e) {
    const cached = loadFromCache(filename);
    if (cached && isValidSnapshot(cached.data)) {
      return { data: cached.data, fromCache: true, timestamp: cached.data.generatedAt || cached.cachedAt, error: e.message };
    }
    throw new Error(`No internet connection and no valid cached data available yet. (${e.message})`);
  }
}
...
// Desktop syncs to Drive roughly every 35 min, so data older than that
// means the desktop hasn't pushed a fresh sync yet — show "Offline" with
// the actual last-update time instead of a misleading "Live" badge, even
// if the phone's fetch from Drive itself succeeded.
const LIVE_WINDOW_MS = 35 * 60 * 1000;

function syncBadge(fromCache, timestamp) {
  const dataTime = timestamp ? new Date(timestamp).getTime() : NaN;
  const isFresh = !isNaN(dataTime) && (Date.now() - dataTime) <= LIVE_WINDOW_MS;
  return isFresh
...
function renderTabContent(data, tab) {
  ...
  if (tab === 'alerts') {
    // Defensive defaults: a stale/partial cached snapshot (old format, failed
    // write, etc.) may not have `alerts` at all, or may be missing a bucket —
    // fall back to empty arrays rather than crashing the whole tab.
    const rawAlerts = data.alerts || {};
    const a = {
      expired: rawAlerts.expired || [],
      outOfStock: rawAlerts.outOfStock || [],
      expiring30: rawAlerts.expiring30 || [],
      expiring60: rawAlerts.expiring60 || [],
      lowStock: rawAlerts.lowStock || [],
    };
